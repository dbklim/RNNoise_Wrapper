...
Сбалансированная чистая речь будет в папке `all_balanced_speech`. Исходная структура аудиозаписей нарушается (все аудиозаписи будут внутри указанной папки, без подпапок), а исходные имена аудиозаписей сохраняются.
...
Подготовить виртуальное окружение Python для обучения:
...
1. Копирование папок вместе с содержимым:

```bash
cp -R source source_copy
```

2. Перемещение папок вместе с содержимым:

```bash
mv -v source_1 source_2 source_N all_source
```

3. Копирование 10 случайных файлов из текущей папки:

```bash
ls | shuf -n 10 | xargs -i cp {} /datasets/random_audio
```

4. Подсчёт размера размер папки:

```bash
du -hs datasets/clean
```

5. Подсчёт количества файлов в текущей папке и всех её подпапках:

```bash
ls -laR | grep "^-" | wc
```

6. Архивировать папку в .zip:

```bash
zip -r training_set_oct30_5h.zip datasets/training_set_oct30_5h
```

7. Распаковать архив в текущую папку:

```bash
unzip training_set_oct30_5h.zip
```
